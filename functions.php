...
if ( ! function_exists( 'peace_setup' ) ) :
	...
	function peace_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'peace' ),
			)
		);
...
function peace_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'peace_content_width', 640 );
}
add_action( 'after_setup_theme', 'peace_content_width', 0 );
...
function peace_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'peace' ),
...
function peace_scripts() {
	...
	wp_enqueue_script( 'peace-popup-js', get_template_directory_uri() . '/js/jquery.magnific-popup.min.js', 'jquery', _S_VERSION, true );
    wp_enqueue_script( 'peace-script', get_template_directory_uri() . '/js/main.js', array(), _S_VERSION, true );
    
    wp_localize_script( 'main-script', 'wpAjax', 
